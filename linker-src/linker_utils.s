...
inst_needs_relocation:
	# YOUR CODE HERE	
	srl $a0,$a0,26
	beq $a0,2,relocation_needed
	beq $a0,3,relocation_needed
	j relocation_not_needed
	
relocation_needed:
	addiu $v0,$0,1
	j inst_needs_relocation_exit

relocation_not_needed:
	addiu $v0,$0,0
...
# on the given symbol and relocation tables.
#
...
relocate_inst:
	...
	add $s0,$0,$a0			#  $s0 = an instruction that needs relocating
	add $s1,$0,$a1			#  $s1 = the byte offset of the instruction in the current file
	add $s2,$0,$a2			#  $s2 = the symbol table
	add $s3,$0,$a3			#  $s3 = the relocation table
	
	add $a0,$0,$s3
	add $a1,$0,$s1
	jal symbol_for_addr		# Look for the addr in relocation table
	beqz $v0,relocate_inst_error	# Error if not found
	
	add $a0,$0,$s2
	add $a1,$0,$v0
	jal addr_for_symbol		# Look for the name in symbol table
	beq $v0,-1,relocate_inst_error	# Error if not found
	
	srl $v0,$v0,2
	srl $s0,$s0,26			# Clear garbage 
	sll $s0,$s0,26
	add $v0,$s0,$v0
