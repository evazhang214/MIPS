...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    if (strcmp(name, "li") == 0 && num_args == 2) {
    	long int imm;
    	if (translate_num(&imm, args[1], INT16_MIN, INT16_MAX) != -1) {
    		fprintf(output, "addiu %s $0 %s\n", args[0], args[1]);
    		return 1;
    	} else if (translate_num(&imm, args[1], INT32_MIN, UINT32_MAX) != -1) {
    ...
    } else if (strcmp(name, "mul") == 0 && num_args == 3) {
    	fprintf(output, "mult %s %s\n", args[1], args[2]);
    	fprintf(output, "mflo %s\n", args[0]);
        return 2;
    } else if (strcmp(name, "quo") == 0 && num_args == 3) {
    	fprintf(output, "div %s %s\n", args[1], args[2]);
    	fprintf(output, "mflo %s\n", args[0]);
        return 2;
    } else if (strcmp(name, "rem") == 0 && num_args == 3) {
    	fprintf(output, "div %s %s\n", args[1], args[2]);
...
/* Writes the instruction in hexadecimal format to OUTPUT during pass #2.
   ...
   Some function declarations for the write_*() functions are provided in translate.h, and you
   MUST implement these and use these as function headers. You may use helper functions,
   but you still must implement the provided write_* functions declared in translate.h.
...
/* Returns instruction code for R-type instructions. */
uint32_t r_instruction(uint32_t opcode, int rs, int rt, int rd, long int shamt, uint32_t funct) {
...
/* Returns instruction code for I-type instructions. */
uint32_t i_instruction(uint32_t opcode, int rs, int rt, long int imm) {
	uint32_t instruction = opcode;
	instruction <<= 5;
	instruction += rs;
	instruction <<= 5;
...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
	if (num_args == 3) {
		...
		uint32_t instruction = r_instruction(0, rs, rt, rd, 0, funct);
		...
		uint32_t instruction = r_instruction(0, 0, 0, rd, 0, funct);
		write_inst_hex(output, instruction);
		return 0;
	} else {
		return -1;
...
int write_shift(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = r_instruction(0, 0, rt, rd, shamt, funct);
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_jr(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = r_instruction(0, rs, 0, 0, 0, funct);
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_addiu(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = i_instruction(opcode, rs, rt, imm);
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_lui(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = i_instruction(opcode, 0, rt, imm);
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_branch(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl) {
    ...
    int32_t offset = (label_addr - addr) / 4 - 1;
    uint32_t instruction = i_instruction(opcode, rs, rt, offset);
    write_inst_hex(output, instruction);        
    return 0;
}

int write_jump(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* reltbl) {
	if (num_args != 1 || !is_valid_label(args[0]) || add_to_table(reltbl, args[0], addr) == -1) {
		return -1;
	}
